...
[xx, yy, realCost{j+J}, latent{j}] = ...
  rollout_real(policy, HH, plant, cost);
...
if plotting.verbosity > 0
  ...
  hold on; plot(1:length(realCost{J+j}),realCost{J+j},'r'); title('Cost of current rollout'); drawnow;
end
...
if plotting.verbosity > 1
  lat = cell(1,10);
  for i=1:10
    [~,~,~,lat{i}] = rollout_real(policy, HH, plant, cost);
  end
