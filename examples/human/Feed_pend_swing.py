...
from utils_human import rollout_feed, rollout_both
...
if __name__=='__main__':
    ...
    X, Y, _, _ = rollout_feed(env, None, timesteps=T_sim, random=True, SUBS=SUBS, render=True)
    for i in range(1,J):
        X_, Y_, _, _= rollout_feed(env, None, timesteps=T_sim,random=True, SUBS=SUBS, render=True)
        X = np.vstack((X, X_))
        Y = np.vstack((Y, Y_))
    ...
    for rollouts in range(N):
        ...
        X_new, Y_new, _, _ = rollout_feed(env, pilco, timesteps=T_sim, SUBS=SUBS, sig=True, render=True)
        X_re, _, _, _ = rollout(env_re, pilco, timesteps=T_sim, SUBS=SUBS, render=False)

        # Since we had decide on the various parameters of the reward function
        # we might want to verify that it behaves as expected by inspection
        for i in range(len(X_re)):
                r_new[:, 0] = R.compute_reward(X_re[i,None,:-1], 0.001 * np.eye(state_dim))[0]
        total_r = sum(r_new)
        _, _, r = pilco.predict(X_re[0,None,:-1], 0.001 * S_init, T)
    ...
    np.save('./plot/Feed_swing_pend_X4.npy', count)
    np.save('./plot/Feed_pend_Y4.npy', re_p)
    np.save('./plot/Feed_pend_Yn4.npy', re_pn)