...
if __name__=='__main__':
    ...
    predicted_r_changed = np.zeros(N2+1)
    r_new = np.zeros((T, 1))
    for rollouts in range(N2):
        print("**** ITERATION no", rollouts, " ****")
    ...
    for i in range(T):
        r_new[i, 0] = R.compute_reward(X_new[i,None,:-1], 0.001 * np.eye(state_dim))[0]
    total_r_changed[N2] = sum(r_new)

    # Predict the reward for the total rollout from the initial state from the model
    _, _, predicted_r_changed[N2] = pilco.predict(X_new[0,None,:-1], 0.001 * S_init, T_sim)

    # The difference gives us an idea of the convergence
    print("Total ", total_r_changed[N2], " Predicted: ", predicted_r_changed[N2])



    np.savez('./logs/' + timeStr + '/rewards.npz', total_r=total_r, predicted_r=predicted_r, \
        total_r_changed=total_r_changed, predicted_r_changed=predicted_r_changed)
