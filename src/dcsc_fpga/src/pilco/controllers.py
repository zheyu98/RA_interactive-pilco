...
class RbfController(MGPR):
    ...
    def compute_action(self, m, s, flag=0, squash=True):
        '''
        ...
        with tf.name_scope("controller") as scope:
            if flag==0: iK, beta = self.calculate_factorizations()
            else: iK = self.iK; beta = self.beta
...
class HumanController(gpflow.Module):
    '''
    A Human Controller with GP models
    '''
    def __init__(self, data, state_dim, max_action=1.0):
        gpflow.Module.__init__(self)
        self.max_action = max_action
        self.model = MGPR(data)
        self.state_dim = state_dim

    def optimize_models(self, maxiter=200, restarts=1, op=True):
        '''
        Optimize GP models
        '''
        if op: self.model.optimize(restarts=restarts)
    ...
    def compute_action(self, m, s, squash=True):
        ...
        '''
        with tf.name_scope("controller") as scope:
            iK, beta = self.model.calculate_factorizations()
            M, S, V = self.model.predict_given_factorizations(m, s, 0.0 * iK, beta)
            S = S - tf.linalg.diag(self.model.variance - 1e-6)
        if squash:
            M, S, V2 = squash_sin(M, S, self.max_action)
            V = V @ V2
        return M, S, V
