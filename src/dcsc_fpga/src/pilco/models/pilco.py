...
class PILCO(gpflow.models.BayesianModel):
    ...
    def optimize_models(self, maxiter=200, restarts=1, op=True):
        ...
        Optimize GP models
        '''
        if op: self.mgpr.optimize(restarts=restarts)
    ...
    def optimize_policy(self, maxiter=50, restarts=1):
        ...
        _, _ = self.controller.calculate_factorizations()
    ...
    def compute_final_action(self, x_m):
        return self.controller.compute_action(x_m, tf.zeros([self.state_dim, self.state_dim], float_type),flag=1)[0]
