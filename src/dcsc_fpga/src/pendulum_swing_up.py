...
def rollout_last(env, pilco, timesteps, verbose=True, random=False, SUBS=1, render=False):
        X = []; Y = []; counter = 0
        x = env.reset()
        ep_return_full = 0
        ep_return_sampled = 0
...
if __name__=='__main__':
    ...
    X_new, Y_new, _, _ = rollout_last(env, pilco, timesteps=T_sim, verbose=True, SUBS=SUBS, render=True)
